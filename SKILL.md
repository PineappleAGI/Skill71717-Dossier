...
- Mode 1 is a single-page PRISMA flow (intake → restatement → search → extract → synthesis → briefing). Search starts as soon as the restatement is shown; there is no “yes that’s right” gate. **Use my correction** reruns the live search from the rewritten question. Step 3 stacks search hits core, then related, then lower-ranked (not three side-by-side columns) with a match-strength bar and scannable evidence cards. Every evidence card shows Background and Objective from the abstract when present. Step 4 (extracted evidence) uses labeled PICO cards instead of a wide table and shows 6 cards with the rest collapsed. Step 5 (synthesis) uses a three-way quality-weighted bar (supporting / contradicting / open questions) and scannable evidence cards, not a bullet list. The last step is a short briefing (headline, short answer, close) built only from retrieved abstracts — no paper walkthroughs or further-reading lists — plus a light, engaging blog essay about the question itself (not an evidence-count or trustworthiness recap), with an editorial photo and/or sketch graph when the topic supports it (captioned as not a study result), and a **Download PDF** control that saves a `.pdf` file (never `window.print()`, which crashes Cursor). The research question is interpreted in Phase 1b before the page loads; Mode 1 does not offer a second question rewrite or a Boolean editor. The query used is shown on the results list. It must be served via `scripts/mode1-server.py` so PubMed / Europe PMC / trials / OpenAlex / Unpaywall calls work.
