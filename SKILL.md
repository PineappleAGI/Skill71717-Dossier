...
- The dossier is a single Mode 1 PRISMA flow. The research question comes from the Phase 1 form (`original_topic` / interpreted `topic`). **Ask another question** (top-right of the page, primary button) opens a fresh intake form at `http://127.0.0.1:8767/ask` on the Mode 1 server; after submit, wait for a new `raw_submission.json` with `wait-for-submission.py` (it ignores a prior run’s file) and rerun Phase 1b → harvest → generate → serve. The PRISMA bar (**This follows PRISMA** + Restatement / Results / Briefing) sticks to the top after you scroll past it and highlights the section in view. The page opens with a light, engaging **blog essay about the question itself** (not an evidence-count recap), with an editorial photo and/or sketch graph when the topic supports it (captioned as not a study result). Share controls: a **Copy article** button sits next to the byline date and copies the full essay as plain text for Medium / Substack; **Download cover** saves a 1200×630 PNG for LinkedIn/X; **Download as Image** saves a tall PNG of the entire essay (never `window.print()`, which crashes Cursor); **Download PDF** remains as a secondary export. Paper exploration follows: restatement → **results** → briefing. Under **See the results**, the stepper shows the PRISMA work behind that step — **Search** (identification) → **Screen & rank** → **Extract** → **Evaluate** — and highlights the live stage. Search, ranking, extraction, and weighing run in the background (status lives in the stepper). Restatement has no “Not quite” / “Use my correction” gate. The user-facing Results step is one view: a recap of that pipeline, a two-way quality-weighted supporting / contradicting bar plus scannable evidence cards (Background and Objective from abstract sentences that overlap the question), then a **Related papers** block of highly cited / major-journal works in the same field (OpenAlex `cited_by_count`, not news or forum rankings; not a direct test of the exact question). Related papers show **two rows**, then **Show more**. Unclear abstracts are omitted from the bar. Lower-ranked search hits are dropped. Search starts as soon as the restatement is shown. The briefing is a short abstract-only writeup (headline, short answer, close) ending with a **Beta posterior + 95% credible interval** graph of the scan’s supporting vs contradicting weights (labeled as not a meta-analytic effect). No paper walkthroughs, further-reading lists, or a second copy of the blog. Mode 1 does not offer a second question rewrite or a Boolean editor. The query used is shown on the results list. It must be served via `scripts/mode1-server.py` so PubMed / Europe PMC / trials / OpenAlex / Unpaywall calls work.
